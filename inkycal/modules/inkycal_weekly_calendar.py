...
class WeeklyCalendar(inkycal_module):
  """Calendar class
  Create monthly calendar and show events from given icalendars
  """

  name = "Weekly Calendar - Show next seven days with events from iCalendars"
  ...
  def generate_image(self):
    ...
    events = parser.get_events(now, now.shift(weeks=7), self.timezone)
    lang = self.language
    ...
    for event in events:
      ...
      write(im_black, (50, 50 + i),
            (300, 40), name, font=self.font,
            alignment = 'left')
      i = i + 50
      logger.info(f'Rendered event {name}')

    # return the images ready for the display
    #return im_black.rotate(-90, expand=True), im_colour.rotate(-90, expand=True)
    return im_black, im_colour
